...
// ── HOOK: IPC — API JSON INEbase tabla 50902 (índices acumulados, base 2021) ──
function useIPC() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Tabla 50902 = Índices nacionales: general y de grupos ECOICOP
        // tip=A devuelve nombres legibles; nult=36 = últimos 36 meses
        ...
        // json = array de series. Cada serie tiene: Nombre, COD, Data[]
        // Filtramos las que son tipo "Índice" (no variación mensual ni anual)
        // y que corresponden a cada grupo ECOICOP que nos interesa
        const resultado = {};
        for (const [catNombre, grupoBuscar] of Object.entries(ECOICOP_GRUPOS)) {
          const serie = json.find(s =>
            s.Nombre && 
            s.Nombre.toLowerCase().includes(grupoBuscar.toLowerCase().slice(0, 15)) &&
            s.Nombre.toLowerCase().includes("índice") &&
            !s.Nombre.toLowerCase().includes("variación") &&
            !s.Nombre.toLowerCase().includes("variacion")
          );
          if (serie && serie.Data) {
            resultado[catNombre] = serie.Data
              .filter(d => d.Valor != null && d.Valor > 10) // los índices son > 50, variaciones < 10
              .map(d => ({ mes: fmtFecha(d.Fecha), valor: +d.Valor.toFixed(2), ts: d.Fecha }))
              .sort((a, b) => a.ts - b.ts);
          }
        }

        // Si no encontramos datos suficientes, lanzar error para usar fallback
        if (Object.keys(resultado).length < 2) throw new Error("Series no encontradas");
        setDatos(resultado);
      } catch (e) {
        setError(e.message);
        setDatos({
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text,
      ...
      <div style={{ borderBottom:`1px solid ${C.border}`, padding:"16px 24px",
        ...
        <div style={{ background:`${C.accent}15`, border:`1px solid ${C.accent}30`,
          borderRadius:20, padding:"4px 14px", fontSize:11, color:C.accent }}>
          🟢 EN VIVO — INE · REE
        </div>
      </div>
